#!/usr/bin/env node
/**
 * Push schema, then pull and verify austinTrailSegments_v2 and austinTrailSystems_v2 in pulled file.
 */
...
(async () => {
  try {
    await run("npx", ["instant-cli@latest", "push", "--app", appId, "--yes"]);
  } catch (e) {
  ...
  const schemaPathToRead = resolve(root, "src/lib/instant/schema.ts");
  const pathToRead = existsSync(resolvedPath) ? resolvedPath : schemaPathToRead;
  ...
  if (!allPresent) {
    console.error("\nOne or more canonical entities missing in schema. Fix schema and re-push.");
    process.exit(1);
  }
  console.log("\nAll 4 canonical entities present.");
})();
