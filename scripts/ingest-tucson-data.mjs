#!/usr/bin/env node
/**
 * Ingest Tucson, AZ trail segments from OpenStreetMap via Overpass API.
...
// Tucson, AZ bbox: S,W,N,E — covers Saguaro NP, Rillito, Pantano, Tucson Mountain Park
const CITY_BBOX = "32.06,-111.10,32.40,-110.70";
const CITY_NAME = "Tucson";
const CITY_STATE = "AZ";
const CITY_COUNTY = "Pima";
const EXT_DATASET = "tucson_osm";
...
const TRAIL_INDICATORS = /\b(trail|trails|greenway|greenways|path|hike|bikeway|corridor|lake|creek|river|mountain|park|loop|canyon|arroyo|wash|ridge)\b/i;
...
const NORMALIZE_NAMES = new Map([
  ["rillito regional park trail", "Rillito Regional Park Trail"],
  ["pantano river park trail", "Pantano River Park Trail"],
  ["julian wash greenway", "Julian Wash Greenway"],
]);
...
async function fetchOSMTrails() {
  const trailNameRegex = "trail|trails|greenway|greenways|hike|bikeway|corridor|lake|creek|river|mountain|park|loop|canyon|arroyo|wash|ridge";
  const query = [
