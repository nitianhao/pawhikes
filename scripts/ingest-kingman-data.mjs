#!/usr/bin/env node
/**
 * Ingest Kingman, AZ trail segments from OpenStreetMap via Overpass API.
...
// Kingman, AZ bbox: S,W,N,E — covers Hualapai Mountain Park, Andy Devine
const CITY_BBOX = "35.15,-114.15,35.30,-113.95";
const CITY_NAME = "Kingman";
const CITY_STATE = "AZ";
const CITY_COUNTY = "Mohave";
const EXT_DATASET = "kingman_osm";
...
const TRAIL_INDICATORS = /\b(trail|trails|greenway|greenways|path|hike|bikeway|corridor|lake|creek|river|mountain|park|loop|canyon|ridge|hualapai)\b/i;
...
const NORMALIZE_NAMES = new Map([
  ["hualapai mountain trail", "Hualapai Mountain Trail"],
]);
...
async function fetchOSMTrails() {
  const trailNameRegex = "trail|trails|greenway|greenways|hike|bikeway|corridor|lake|creek|river|mountain|park|loop|canyon|ridge|hualapai";
  const query = [
