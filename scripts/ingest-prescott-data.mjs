#!/usr/bin/env node
/**
 * Ingest Prescott, AZ trail segments from OpenStreetMap via Overpass API.
...
// Prescott, AZ bbox: S,W,N,E — covers Prescott National Forest, Thumb Butte, Willow Lake
const CITY_BBOX = "34.50,-112.55,34.65,-112.35";
const CITY_NAME = "Prescott";
const CITY_STATE = "AZ";
const CITY_COUNTY = "Yavapai";
const EXT_DATASET = "prescott_osm";
...
const TRAIL_INDICATORS = /\b(trail|trails|greenway|greenways|path|hike|bikeway|corridor|lake|creek|river|mountain|park|loop|canyon|ridge|forest|butte)\b/i;
...
const NORMALIZE_NAMES = new Map([
  ["thumb butte trail", "Thumb Butte Trail"],
  ["prescott circle trail", "Prescott Circle Trail"],
]);
...
async function fetchOSMTrails() {
  const trailNameRegex = "trail|trails|greenway|greenways|hike|bikeway|corridor|lake|creek|river|mountain|park|loop|canyon|ridge|forest|butte";
  const query = [
