#!/usr/bin/env node
/**
 * Ingest Lake Havasu City, AZ trail segments from OpenStreetMap via Overpass API.
...
// Lake Havasu City, AZ bbox: S,W,N,E — covers SARA Park, Mohave Wash, waterfront trail
const CITY_BBOX = "34.40,-114.45,34.60,-114.25";
const CITY_NAME = "Lake Havasu City";
const CITY_STATE = "AZ";
const CITY_COUNTY = "Mohave";
const EXT_DATASET = "lake_havasu_city_osm";
...
const TRAIL_INDICATORS = /\b(trail|trails|greenway|greenways|path|hike|bikeway|corridor|lake|creek|river|mountain|park|loop|canyon|wash|havasu)\b/i;
...
const NORMALIZE_NAMES = new Map([
  ["sara park trail", "SARA Park Trail"],
  ["mohave wash trail", "Mohave Wash Trail"],
]);
...
async function fetchOSMTrails() {
  const trailNameRegex = "trail|trails|greenway|greenways|hike|bikeway|corridor|lake|creek|river|mountain|park|loop|canyon|wash|havasu";
  const query = [
