#!/usr/bin/env node
/**
 * Ingest Casa Grande, AZ trail segments from OpenStreetMap via Overpass API.
...
// Casa Grande, AZ bbox: S,W,N,E — covers Pinal Air Park area, Dave White Regional Park
const CITY_BBOX = "32.82,-111.85,32.98,-111.65";
const CITY_NAME = "Casa Grande";
const CITY_STATE = "AZ";
const CITY_COUNTY = "Pinal";
const EXT_DATASET = "casa_grande_osm";
...
const TRAIL_INDICATORS = /\b(trail|trails|greenway|greenways|path|hike|bikeway|corridor|lake|creek|river|mountain|park|loop|canal|arroyo)\b/i;
...
const NORMALIZE_NAMES = new Map([
  ["dave white regional park trail", "Dave White Regional Park Trail"],
]);
...
async function fetchOSMTrails() {
  const trailNameRegex = "trail|trails|greenway|greenways|hike|bikeway|corridor|lake|creek|river|mountain|park|loop|canal|arroyo";
  const query = [
