#!/usr/bin/env node
/**
 * Ingest Flagstaff, AZ trail segments from OpenStreetMap via Overpass API.
...
// Flagstaff, AZ bbox: S,W,N,E — covers Buffalo Park, Mt Elden, Walnut Canyon
const CITY_BBOX = "35.10,-111.75,35.25,-111.55";
const CITY_NAME = "Flagstaff";
const CITY_STATE = "AZ";
const CITY_COUNTY = "Coconino";
const EXT_DATASET = "flagstaff_osm";
...
const TRAIL_INDICATORS = /\b(trail|trails|greenway|greenways|path|hike|bikeway|corridor|lake|creek|river|mountain|park|loop|canyon|ridge|forest)\b/i;
...
const NORMALIZE_NAMES = new Map([
  ["buffalo park trail", "Buffalo Park Trail"],
  ["mount elden trail", "Mount Elden Trail"],
]);
...
async function fetchOSMTrails() {
  const trailNameRegex = "trail|trails|greenway|greenways|hike|bikeway|corridor|lake|creek|river|mountain|park|loop|canyon|ridge|forest";
  const query = [
