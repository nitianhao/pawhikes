#!/usr/bin/env node
/**
 * Ingest Yuma, AZ trail segments from OpenStreetMap via Overpass API.
...
// Yuma, AZ bbox: S,W,N,E — covers Colorado River path, Gateway Park
const CITY_BBOX = "32.55,-114.75,32.80,-114.45";
const CITY_NAME = "Yuma";
const CITY_STATE = "AZ";
const CITY_COUNTY = "Yuma";
const EXT_DATASET = "yuma_osm";
...
const TRAIL_INDICATORS = /\b(trail|trails|greenway|greenways|path|hike|bikeway|corridor|lake|creek|river|mountain|park|loop|canyon|levee)\b/i;
...
const NORMALIZE_NAMES = new Map([
  ...
  ["gateway park trail", "Gateway Park Trail"],
]);
...
async function fetchOSMTrails() {
  const trailNameRegex = "trail|trails|greenway|greenways|hike|bikeway|corridor|lake|creek|river|mountain|park|loop|canyon|levee";
  const query = [
