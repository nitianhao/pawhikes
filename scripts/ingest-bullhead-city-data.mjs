#!/usr/bin/env node
/**
 * Ingest Bullhead City, AZ trail segments from OpenStreetMap via Overpass API.
...
// Bullhead City, AZ bbox: S,W,N,E — covers Colorado River waterfront, Laughlin area
const CITY_BBOX = "35.05,-114.65,35.25,-114.45";
const CITY_NAME = "Bullhead City";
const CITY_STATE = "AZ";
const CITY_COUNTY = "Mohave";
const EXT_DATASET = "bullhead_city_osm";
...
const TRAIL_INDICATORS = /\b(trail|trails|greenway|greenways|path|hike|bikeway|corridor|lake|creek|river|mountain|park|loop|canyon|colorado)\b/i;
...
const NORMALIZE_NAMES = new Map([
  ["colorado river trail", "Colorado River Trail"],
]);
...
async function fetchOSMTrails() {
  const trailNameRegex = "trail|trails|greenway|greenways|hike|bikeway|corridor|lake|creek|river|mountain|park|loop|canyon|colorado";
  const query = [
