#!/usr/bin/env node
/**
 * Ingest Sedona, AZ trail segments from OpenStreetMap via Overpass API.
...
// Sedona, AZ bbox: S,W,N,E — covers Red Rock State Park, Bell Rock, Cathedral Rock
const CITY_BBOX = "34.80,-111.95,34.95,-111.75";
const CITY_NAME = "Sedona";
const CITY_STATE = "AZ";
const CITY_COUNTY = "Yavapai";
const EXT_DATASET = "sedona_osm";
...
const TRAIL_INDICATORS = /\b(trail|trails|greenway|greenways|path|hike|bikeway|corridor|lake|creek|river|mountain|park|loop|canyon|ridge|rock|mesa|butte)\b/i;
...
const NORMALIZE_NAMES = new Map([
  ["bell rock trail", "Bell Rock Trail"],
  ["cathedral rock trail", "Cathedral Rock Trail"],
  ["devil's bridge trail", "Devil's Bridge Trail"],
]);
...
async function fetchOSMTrails() {
  const trailNameRegex = "trail|trails|greenway|greenways|hike|bikeway|corridor|lake|creek|river|mountain|park|loop|canyon|ridge|rock|mesa|butte";
  const query = [
