#!/usr/bin/env npx tsx
/**
 * Seed official dog policy fields onto Bullhead City, AZ trailSystems.
 * Applies BULLHEAD_CITY_DEFAULT_POLICY to all systems in the city.
 *
 * DRY RUN by default — no DB writes until --commit is passed.
 *
 * Usage:
 *   npx tsx scripts/policy/seed-policy-bullhead-city.ts [--commit]
 */
...
const cityFilter = "Bullhead City";
...
const DEFAULT_POLICY: Policy = {
  ...
  leashDetails: "Dogs must be on leash in all Bullhead City parks and along the Colorado River trail. Bullhead City Code §91.003.",
  policySourceUrl: "https://www.bullheadcity.com/departments/parks-recreation",
  policySourceTitle: "Parks and Recreation | City of Bullhead City",
  policyConfidence: 0.75,
  policyMethod: "manual_seed",
  policyNotes: "Bullhead City parks require leash. Colorado River trail also requires leash.",
};

async function main(): Promise<void> {
  console.log("=== seed-policy-bullhead-city ===");
  ...
  console.log(`Found ${systems.length} Bullhead City trailSystems.\n`);
