#!/usr/bin/env npx tsx
/**
 * Seed official dog policy fields onto Bakersfield, CA trailSystems.
 *
 * DRY RUN by default — no DB writes until --commit is passed.
 *
 * Usage:
 *   npx tsx scripts/policy/seed-policy-bakersfield.ts [--commit]
 */
...
const DEFAULT_POLICY: Policy = {
  ...
  leashDetails: "Dogs must be on a leash in all Bakersfield city parks. Bakersfield Municipal Code §6.08.140.",
  policySourceUrl: "https://www.bakersfieldcity.us/518/Parks",
  policySourceTitle: "Parks | City of Bakersfield Recreation and Parks",
  policyConfidence: 0.80,
  policyMethod: "manual_seed",
  policyNotes: "Bakersfield city parks require leash. Kern River Parkway trail allows leashed dogs. Designated off-leash dog parks at various locations (University Park, Kroll Park).",
};

async function main(): Promise<void> {
  console.log("=== seed-policy-bakersfield ===");
  ...
  console.log(`Found ${systems.length} Bakersfield trailSystems.\n`);
