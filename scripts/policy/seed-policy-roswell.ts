#!/usr/bin/env npx tsx
/**
 * Seed official dog policy fields onto Roswell, NM trailSystems.
 * DRY RUN by default — no DB writes until --commit is passed.
 * Usage: npx tsx scripts/policy/seed-policy-roswell.ts [--commit]
 */
...
const DEFAULT_POLICY: Policy = {
  ...
  leashDetails: "Dogs must be on leash in Roswell city parks and greenway trails. Roswell City Code §6-2-3.",
  policySourceUrl: "https://www.roswell-nm.gov/departments/parks_and_recreation",
  policySourceTitle: "Parks and Recreation | City of Roswell",
  policyConfidence: 0.75,
  policyMethod: "manual_seed",
  policyNotes: "Roswell City Code §6-2-3 requires leash at all times in city parks and trails.",
};

async function main(): Promise<void> {
  console.log("=== seed-policy-roswell ===");
