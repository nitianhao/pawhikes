#!/usr/bin/env npx tsx
/**
 * Seed official dog policy fields onto San Diego, CA trailSystems.
 * Applies SD_DEFAULT_POLICY to all systems in the city.
 *
 * DRY RUN by default — no DB writes until --commit is passed.
 *
 * Usage:
 *   npx tsx scripts/policy/seed-policy-san-diego.ts [--commit]
 */
...
const DEFAULT_POLICY: Policy = {
  ...
  leashDetails: "Dogs must be on a leash no longer than 8 feet in all San Diego city parks. SDMC §63.0102.",
  policySourceUrl: "https://www.sandiego.gov/park-and-recreation/parks/dogs",
  policySourceTitle: "Dogs in Parks | City of San Diego",
  policyConfidence: 0.80,
  policyMethod: "manual_seed",
  policyNotes: "San Diego allows 8-ft leash in city parks. Fiesta Island has a large off-leash area. Dogs prohibited on most beaches except designated dog beaches (Ocean Beach Dog Beach, Coronado Dog Beach).",
};

async function main(): Promise<void> {
  console.log("=== seed-policy-san-diego ===");
  ...
  console.log(`Found ${systems.length} San Diego trailSystems.\n`);
