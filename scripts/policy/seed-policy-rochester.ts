#!/usr/bin/env npx tsx
/**
 * Seed official dog policy fields onto Rochester, NY trailSystems.
 *
 * DRY RUN by default — no DB writes until --commit is passed.
 *
 * Usage:
 *   npx tsx scripts/policy/seed-policy-rochester.ts [--commit]
 */
...
const DEFAULT_POLICY: Policy = {
  ...
  leashDetails: "Dogs must be on a leash no longer than 6 feet in Rochester city parks. Rochester City Code §60-13. Dogs must be under control at all times.",
  policySourceUrl: "https://www.cityofrochester.gov/parks/",
  policySourceTitle: "Parks & Recreation | City of Rochester",
  policyConfidence: 0.80,
  policyMethod: "manual_seed",
  policyNotes: "Rochester city parks require 6-ft leash. Genesee Riverway Trail and Erie Canalway Trail are popular dog-friendly routes. Durand Eastman Park allows leashed dogs. Several designated off-leash dog parks available.",
};

async function main(): Promise<void> {
  console.log("=== seed-policy-rochester ===");
  ...
  console.log(`Found ${systems.length} Rochester trailSystems.\n`);
