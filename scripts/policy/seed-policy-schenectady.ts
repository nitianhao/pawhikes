#!/usr/bin/env npx tsx
/**
 * Seed official dog policy fields onto Schenectady, NY trailSystems.
 *
 * DRY RUN by default — no DB writes until --commit is passed.
 *
 * Usage:
 *   npx tsx scripts/policy/seed-policy-schenectady.ts [--commit]
 */
...
const DEFAULT_POLICY: Policy = {
  ...
  leashDetails: "Dogs must be on a leash no longer than 6 feet in Schenectady city parks. Dogs must be licensed and vaccinated. Owners are responsible for cleanup.",
  policySourceUrl: "https://www.cityofschenectady.com/parks",
  policySourceTitle: "Parks | City of Schenectady",
  policyConfidence: 0.75,
  policyMethod: "manual_seed",
  policyNotes: "Schenectady parks require leashed dogs. Mohawk-Hudson Bike-Hike Trail (along Erie Canalway) allows leashed dogs — runs through the city along the Mohawk River. Central Park allows leashed dogs. Vale Park (Schenectady County) allows leashed dogs.",
};

async function main(): Promise<void> {
  console.log("=== seed-policy-schenectady ===");
  ...
  console.log(`Found ${systems.length} Schenectady trailSystems.\n`);
