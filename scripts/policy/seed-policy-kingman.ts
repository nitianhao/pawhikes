#!/usr/bin/env npx tsx
/**
 * Seed official dog policy fields onto Kingman, AZ trailSystems.
 * Applies KINGMAN_DEFAULT_POLICY to all systems in the city.
 *
 * DRY RUN by default — no DB writes until --commit is passed.
 *
 * Usage:
 *   npx tsx scripts/policy/seed-policy-kingman.ts [--commit]
 */
...
const DEFAULT_POLICY: Policy = {
  ...
  leashDetails: "Dogs must be on leash in all Kingman city parks. Kingman City Code §6-2. Hualapai Mountain Park (Mohave County) also requires leash.",
  policySourceUrl: "https://www.cityofkingman.gov/government/departments/parks-recreation",
  policySourceTitle: "Parks and Recreation | City of Kingman",
  policyConfidence: 0.75,
  policyMethod: "manual_seed",
  policyNotes: "Kingman City Code §6-2 requires leash. Mohave County parks also require leash.",
};

async function main(): Promise<void> {
  console.log("=== seed-policy-kingman ===");
  ...
  console.log(`Found ${systems.length} Kingman trailSystems.\n`);
