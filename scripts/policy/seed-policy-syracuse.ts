#!/usr/bin/env npx tsx
/**
 * Seed official dog policy fields onto Syracuse, NY trailSystems.
 *
 * DRY RUN by default — no DB writes until --commit is passed.
 *
 * Usage:
 *   npx tsx scripts/policy/seed-policy-syracuse.ts [--commit]
 */
...
const DEFAULT_POLICY: Policy = {
  ...
  leashDetails: "Dogs must be on a leash no longer than 6 feet in Syracuse city parks. Syracuse City Code §53. Dogs must be licensed and vaccinated.",
  policySourceUrl: "https://www.syrgov.net/Parks.aspx",
  policySourceTitle: "Parks | City of Syracuse",
  policyConfidence: 0.78,
  policyMethod: "manual_seed",
  policyNotes: "Syracuse city parks require 6-ft leash. Onondaga Creekwalk allows leashed dogs along the 4-mile paved trail. Onondaga County parks also permit leashed dogs. Green Lakes State Park and Clark Reservation allow leashed dogs on trails.",
};

async function main(): Promise<void> {
  console.log("=== seed-policy-syracuse ===");
  ...
  console.log(`Found ${systems.length} Syracuse trailSystems.\n`);
