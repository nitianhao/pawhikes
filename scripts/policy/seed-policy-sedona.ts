#!/usr/bin/env npx tsx
/**
 * Seed official dog policy fields onto Sedona, AZ trailSystems.
 * Applies SEDONA_DEFAULT_POLICY to all systems in the city.
 *
 * DRY RUN by default — no DB writes until --commit is passed.
 *
 * Usage:
 *   npx tsx scripts/policy/seed-policy-sedona.ts [--commit]
 */
...
const DEFAULT_POLICY: Policy = {
  ...
  leashDetails: "Dogs on leash required on all Sedona city trails and parks. Leash required on National Forest trails (Coconino NF / Prescott NF).",
  policySourceUrl: "https://www.sedonaaz.gov/departments/parks-and-recreation",
  policySourceTitle: "Parks and Recreation | City of Sedona",
  policyConfidence: 0.75,
  policyMethod: "manual_seed",
  policyNotes: "Sedona city parks and adjacent National Forest trails both require leash.",
};

async function main(): Promise<void> {
  console.log("=== seed-policy-sedona ===");
  ...
  console.log(`Found ${systems.length} Sedona trailSystems.\n`);
