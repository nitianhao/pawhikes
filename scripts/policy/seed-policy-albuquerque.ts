#!/usr/bin/env npx tsx
/**
 * Seed official dog policy fields onto Albuquerque, NM trailSystems.
 * Applies DEFAULT_POLICY to all systems in the city.
 *
 * DRY RUN by default — no DB writes until --commit is passed.
 *
 * Usage:
 *   npx tsx scripts/policy/seed-policy-albuquerque.ts [--commit]
 */
...
const DEFAULT_POLICY: Policy = {
  ...
  leashDetails: "Dogs must be on a leash in all City of Albuquerque parks and open spaces. Albuquerque City Code §9-2-1-4.",
  policySourceUrl: "https://www.cabq.gov/parksandrecreation",
  policySourceTitle: "Parks and Recreation | City of Albuquerque",
  policyConfidence: 0.75,
  policyMethod: "manual_seed",
  policyNotes: "Albuquerque City Code §9-2-1-4 requires leash at all times in city parks and open spaces.",
};

async function main(): Promise<void> {
  console.log("=== seed-policy-albuquerque ===");
  ...
  for (const { id, slug, policy } of toWrite) {
    await db.transact([(db.tx as any).trailSystems[id].update({
