#!/usr/bin/env npx tsx
/**
 * Seed official dog policy fields onto Anaheim, CA trailSystems.
 *
 * DRY RUN by default — no DB writes until --commit is passed.
 *
 * Usage:
 *   npx tsx scripts/policy/seed-policy-anaheim.ts [--commit]
 */
...
const DEFAULT_POLICY: Policy = {
  ...
  leashDetails: "Dogs must be on a leash no longer than 6 feet in all Anaheim city parks. Anaheim Municipal Code §6.04.030.",
  policySourceUrl: "https://www.anaheim.net/509/Dog-Parks",
  policySourceTitle: "Dog Parks | City of Anaheim Community Services",
  policyConfidence: 0.80,
  policyMethod: "manual_seed",
  policyNotes: "Anaheim city parks require 6-ft leash. Yorba Regional Park (OC Parks) allows leashed dogs on trails. Santiago Oaks Regional Park allows leashed dogs. Anaheim Hills trails generally allow leashed dogs.",
};

async function main(): Promise<void> {
  console.log("=== seed-policy-anaheim ===");
  ...
  console.log(`Found ${systems.length} Anaheim trailSystems.\n`);
