#!/usr/bin/env npx tsx
/**
 * Seed official dog policy fields onto Tucson, AZ trailSystems.
 * Applies TUCSON_DEFAULT_POLICY to all systems in the city.
 *
 * DRY RUN by default — no DB writes until --commit is passed.
 *
 * Usage:
 *   npx tsx scripts/policy/seed-policy-tucson.ts [--commit]
 */
...
const DEFAULT_POLICY: Policy = {
  ...
  leashDetails: "Dogs must be kept on a leash in all Tucson city parks. Tucson Code of Ordinances §11-31.",
  policySourceUrl: "https://www.tucsonaz.gov/parks-and-recreation",
  policySourceTitle: "Parks and Recreation | City of Tucson",
  policyConfidence: 0.80,
  policyMethod: "manual_seed",
  policyNotes: "Tucson City Code §11-31 requires leash at all times in city parks.",
};

async function main(): Promise<void> {
  console.log("=== seed-policy-tucson ===");
  ...
  console.log(`Found ${systems.length} Tucson trailSystems.\n`);
