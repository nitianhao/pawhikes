#!/usr/bin/env npx tsx
/**
 * Seed official dog policy fields onto Lake Havasu City, AZ trailSystems.
 * Applies LAKE_HAVASU_CITY_DEFAULT_POLICY to all systems in the city.
 *
 * DRY RUN by default — no DB writes until --commit is passed.
 *
 * Usage:
 *   npx tsx scripts/policy/seed-policy-lake-havasu-city.ts [--commit]
 */
...
const cityFilter = "Lake Havasu City";
...
const DEFAULT_POLICY: Policy = {
  ...
  leashDetails: "Dogs must be on leash in all Lake Havasu City parks and trails. Lake Havasu City Code §91.003.",
  policySourceUrl: "https://www.lhcaz.gov/parks-recreation",
  policySourceTitle: "Parks and Recreation | Lake Havasu City",
  policyConfidence: 0.80,
  policyMethod: "manual_seed",
  policyNotes: "Lake Havasu City Code §91.003 requires leash in parks.",
};

async function main(): Promise<void> {
  console.log("=== seed-policy-lake-havasu-city ===");
  ...
  console.log(`Found ${systems.length} Lake Havasu City trailSystems.\n`);
