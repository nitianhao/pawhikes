#!/usr/bin/env npx tsx
/**
 * Seed official dog policy fields onto Prescott, AZ trailSystems.
 * Applies PRESCOTT_DEFAULT_POLICY to all systems in the city.
 *
 * DRY RUN by default — no DB writes until --commit is passed.
 *
 * Usage:
 *   npx tsx scripts/policy/seed-policy-prescott.ts [--commit]
 */
...
const DEFAULT_POLICY: Policy = {
  ...
  leashDetails: "Dogs on leash required in all Prescott city parks. Prescott City Code §3-3-2. Prescott National Forest trails also require leash.",
  policySourceUrl: "https://www.prescott-az.gov/departments/parks-recreation",
  policySourceTitle: "Parks and Recreation | City of Prescott",
  policyConfidence: 0.80,
  policyMethod: "manual_seed",
  policyNotes: "Prescott City Code §3-3-2 requires leash in city parks. National Forest trails adjacent to city also require leash.",
};

async function main(): Promise<void> {
  console.log("=== seed-policy-prescott ===");
  ...
  console.log(`Found ${systems.length} Prescott trailSystems.\n`);
