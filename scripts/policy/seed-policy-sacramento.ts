#!/usr/bin/env npx tsx
/**
 * Seed official dog policy fields onto Sacramento, CA trailSystems.
 *
 * DRY RUN by default — no DB writes until --commit is passed.
 *
 * Usage:
 *   npx tsx scripts/policy/seed-policy-sacramento.ts [--commit]
 */
...
const DEFAULT_POLICY: Policy = {
  ...
  leashDetails: "Dogs must be on a leash no longer than 6 feet in all Sacramento city parks. Sacramento City Code §9.44.380.",
  policySourceUrl: "https://www.cityofsacramento.gov/ParksandRec/Parks/Dog-Parks",
  policySourceTitle: "Dog Parks | City of Sacramento",
  policyConfidence: 0.80,
  policyMethod: "manual_seed",
  policyNotes: "Sacramento city parks require 6-ft leash. American River Parkway (Jedediah Smith Memorial Trail) allows leashed dogs. Several off-leash dog parks available (Granite Regional, Partner Park, Bannon Creek).",
};

async function main(): Promise<void> {
  console.log("=== seed-policy-sacramento ===");
  ...
  console.log(`Found ${systems.length} Sacramento trailSystems.\n`);
