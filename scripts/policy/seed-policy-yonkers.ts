#!/usr/bin/env npx tsx
/**
 * Seed official dog policy fields onto Yonkers, NY trailSystems.
 *
 * DRY RUN by default — no DB writes until --commit is passed.
 *
 * Usage:
 *   npx tsx scripts/policy/seed-policy-yonkers.ts [--commit]
 */
...
const DEFAULT_POLICY: Policy = {
  ...
  leashDetails: "Dogs must be on a leash no longer than 6 feet in Yonkers city parks and trails. Westchester County parks also require 6-ft leash. Dogs must be licensed and under control at all times.",
  policySourceUrl: "https://www.yonkersny.gov/government/departments/parks-recreation",
  policySourceTitle: "Parks & Recreation | City of Yonkers",
  policyConfidence: 0.78,
  policyMethod: "manual_seed",
  policyNotes: "Yonkers parks require leashed dogs. Old Croton Aqueduct Trail (NY State Park) allows leashed dogs throughout. Tibbetts Brook Park (Westchester County) allows leashed dogs. Untermyer Park allows leashed dogs on garden paths.",
};

async function main(): Promise<void> {
  console.log("=== seed-policy-yonkers ===");
  ...
  console.log(`Found ${systems.length} Yonkers trailSystems.\n`);
