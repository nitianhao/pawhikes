#!/usr/bin/env npx tsx
/**
 * Seed official dog policy fields onto South Valley, NM trailSystems.
 * DRY RUN by default — no DB writes until --commit is passed.
 * Usage: npx tsx scripts/policy/seed-policy-south-valley.ts [--commit]
 */
...
const DEFAULT_POLICY: Policy = {
  ...
  leashDetails: "South Valley is an unincorporated community in Bernalillo County. Dogs must be on leash in county parks and open space. Bernalillo County Code §6-3.",
  policySourceUrl: "https://www.bernco.gov/parks-recreation/",
  policySourceTitle: "Parks and Recreation | Bernalillo County",
  policyConfidence: 0.75,
  policyMethod: "manual_seed",
  policyNotes: "Bernalillo County Code §6-3 requires leash at all times in county parks and open space areas.",
};

async function main(): Promise<void> {
  console.log("=== seed-policy-south-valley ===");
