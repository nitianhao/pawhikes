#!/usr/bin/env npx tsx
/**
 * Seed official dog policy fields onto Mount Vernon, NY trailSystems.
 *
 * DRY RUN by default — no DB writes until --commit is passed.
 *
 * Usage:
 *   npx tsx scripts/policy/seed-policy-mount-vernon.ts [--commit]
 */
...
const DEFAULT_POLICY: Policy = {
  ...
  leashDetails: "Dogs must be on a leash no longer than 6 feet in Mount Vernon city parks. Westchester County parks require 6-ft leash. Dogs must be licensed and vaccinated.",
  policySourceUrl: "https://www.mountvernonny.gov/parks-recreation",
  policySourceTitle: "Parks & Recreation | City of Mount Vernon",
  policyConfidence: 0.73,
  policyMethod: "manual_seed",
  policyNotes: "Mount Vernon parks require leashed dogs. Willson's Woods Park (Westchester County) allows leashed dogs. Hutchinson River Parkway greenway accessible for walking. City borders Bronx River Parkway trail corridor.",
};

async function main(): Promise<void> {
  console.log("=== seed-policy-mount-vernon ===");
  ...
  console.log(`Found ${systems.length} Mount Vernon trailSystems.\n`);
