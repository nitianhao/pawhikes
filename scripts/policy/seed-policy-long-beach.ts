#!/usr/bin/env npx tsx
/**
 * Seed official dog policy fields onto Long Beach, CA trailSystems.
 *
 * DRY RUN by default — no DB writes until --commit is passed.
 *
 * Usage:
 *   npx tsx scripts/policy/seed-policy-long-beach.ts [--commit]
 */
...
const DEFAULT_POLICY: Policy = {
  ...
  leashDetails: "Dogs must be on a leash no longer than 6 feet in all Long Beach city parks. LBMC §6.16.010.",
  policySourceUrl: "https://www.longbeach.gov/park/recreation-programs/dogs/",
  policySourceTitle: "Dogs | City of Long Beach Parks, Recreation and Marine",
  policyConfidence: 0.80,
  policyMethod: "manual_seed",
  policyNotes: "Long Beach city parks require 6-ft leash. El Dorado Park has a designated off-leash dog area. Rosie's Dog Beach (between Roycroft & Argonne) allows off-leash dogs on the beach.",
};

async function main(): Promise<void> {
  console.log("=== seed-policy-long-beach ===");
  ...
  console.log(`Found ${systems.length} Long Beach trailSystems.\n`);
