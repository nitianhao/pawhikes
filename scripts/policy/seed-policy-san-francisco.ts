#!/usr/bin/env npx tsx
/**
 * Seed official dog policy fields onto San Francisco, CA trailSystems.
 *
 * DRY RUN by default — no DB writes until --commit is passed.
 *
 * Usage:
 *   npx tsx scripts/policy/seed-policy-san-francisco.ts [--commit]
 */
...
const cityFilter = "San Francisco";
...
const DEFAULT_POLICY: Policy = {
  ...
  leashDetails: "Dogs must be on leash in all San Francisco Recreation & Parks areas unless in a designated off-leash area. SF Park Code §3.02.",
  policySourceUrl: "https://sfrecpark.org/571/Dog-Play-Areas",
  policySourceTitle: "Dog Play Areas | SF Recreation and Parks",
  policyConfidence: 0.80,
  policyMethod: "manual_seed",
  policyNotes: "SF has many designated off-leash areas (Fort Funston, Crissy Field East Beach, Bernal Hill, McLaren Park). GGNRA areas follow NPS rules — leash required on most trails. Golden Gate Park is on-leash except designated DPAs.",
};

async function main(): Promise<void> {
  console.log("=== seed-policy-san-francisco ===");
  ...
  console.log(`Found ${systems.length} San Francisco trailSystems.\n`);
