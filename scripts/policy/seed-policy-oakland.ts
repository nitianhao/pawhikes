#!/usr/bin/env npx tsx
/**
 * Seed official dog policy fields onto Oakland, CA trailSystems.
 *
 * DRY RUN by default — no DB writes until --commit is passed.
 *
 * Usage:
 *   npx tsx scripts/policy/seed-policy-oakland.ts [--commit]
 */
...
const DEFAULT_POLICY: Policy = {
  ...
  leashDetails: "Dogs must be on a leash in all Oakland city parks. Oakland Municipal Code §6.04.090.",
  policySourceUrl: "https://www.oaklandca.gov/topics/dogs-in-parks",
  policySourceTitle: "Dogs in Parks | City of Oakland",
  policyConfidence: 0.80,
  policyMethod: "manual_seed",
  policyNotes: "Oakland city parks require leash. East Bay Regional Park District trails allow leashed dogs; some parks have off-leash areas (Point Isabel, Joaquin Miller). Redwood Regional Park allows leashed dogs on most trails.",
};

async function main(): Promise<void> {
  console.log("=== seed-policy-oakland ===");
  ...
  console.log(`Found ${systems.length} Oakland trailSystems.\n`);
