#!/usr/bin/env npx tsx
/**
 * Seed official dog policy fields onto Alamogordo, NM trailSystems.
 * DRY RUN by default — no DB writes until --commit is passed.
 * Usage: npx tsx scripts/policy/seed-policy-alamogordo.ts [--commit]
 */
...
const DEFAULT_POLICY: Policy = {
  ...
  leashDetails: "Dogs must be on leash in Alamogordo city parks and recreation areas. Alamogordo City Code §6-3. Note: White Sands National Park requires leash on all trails.",
  policySourceUrl: "https://www.ci.alamogordo.nm.us/departments/parks_recreation",
  policySourceTitle: "Parks and Recreation | City of Alamogordo",
  policyConfidence: 0.75,
  policyMethod: "manual_seed",
  policyNotes: "Alamogordo City Code §6-3 requires leash at all times in city parks. White Sands NP also requires leash.",
};

async function main(): Promise<void> {
  console.log("=== seed-policy-alamogordo ===");
