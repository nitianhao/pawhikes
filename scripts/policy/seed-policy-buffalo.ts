#!/usr/bin/env npx tsx
/**
 * Seed official dog policy fields onto Buffalo, NY trailSystems.
 *
 * DRY RUN by default — no DB writes until --commit is passed.
 *
 * Usage:
 *   npx tsx scripts/policy/seed-policy-buffalo.ts [--commit]
 */
...
const DEFAULT_POLICY: Policy = {
  ...
  leashDetails: "Dogs must be on a leash at all times in Buffalo city parks. Buffalo City Code §175-4. Leash must not exceed 6 feet.",
  policySourceUrl: "https://www.buffalony.gov/706/Parks",
  policySourceTitle: "Parks | City of Buffalo",
  policyConfidence: 0.80,
  policyMethod: "manual_seed",
  policyNotes: "Buffalo city parks require leashed dogs. Delaware Park (Olmsted design) is popular with dogs on leash. Tifft Nature Preserve allows leashed dogs on trails. Several off-leash dog parks available in the city.",
};

async function main(): Promise<void> {
  console.log("=== seed-policy-buffalo ===");
  ...
  console.log(`Found ${systems.length} Buffalo trailSystems.\n`);
