#!/usr/bin/env npx tsx
/**
 * Seed official dog policy fields onto Houston trailSystems.
 ...
 *
 * Usage:
 *   npx tsx scripts/policy/seed-policy-houston.ts \
...
const cityFilter = typeof args.city === "string" ? args.city : "Houston";
...
const HOUSTON_POLICY = {
  dogsAllowed: "allowed" as const,
  leashPolicy: "required" as const,
  leashDetails: "Dogs must be on a leash in all Houston parks and bayou greenways. Leash required at all times while on trails.",
  policySourceUrl: "https://www.houstontx.gov/parks/dogs.html",
  policySourceTitle: "Dog-Friendly Parks | City of Houston Parks and Recreation",
  policyConfidence: 0.8,
  policyMethod: "manual_seed" as const,
  policyNotes: "City of Houston parks allow leashed dogs on all trails. Houston Parks and Recreation Department requires leashes in city parks. Blanket policy applied to all Houston trail systems via seed-policy-houston.ts.",
};

async function main(): Promise<void> {
  console.log("=== seed-policy-houston ===");
  ...
  console.log(`Found ${systems.length} Houston trailSystems.\n`);
  ...
  for (const system of systems) {
    ...
    const alreadySet =
      system.dogsAllowed === HOUSTON_POLICY.dogsAllowed &&
      system.leashPolicy === HOUSTON_POLICY.leashPolicy &&
      system.policyConfidence === HOUSTON_POLICY.policyConfidence;
  ...
  const payload: Record<string, unknown> = {
    dogsAllowed: HOUSTON_POLICY.dogsAllowed,
    leashPolicy: HOUSTON_POLICY.leashPolicy,
    leashDetails: HOUSTON_POLICY.leashDetails,
    policySourceUrl: HOUSTON_POLICY.policySourceUrl,
    policySourceTitle: HOUSTON_POLICY.policySourceTitle,
    policyConfidence: HOUSTON_POLICY.policyConfidence,
    policyMethod: HOUSTON_POLICY.policyMethod,
    policyNotes: HOUSTON_POLICY.policyNotes,
