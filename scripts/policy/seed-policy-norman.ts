#!/usr/bin/env npx tsx
/**
 * Seed official dog policy fields onto Norman, OK trailSystems.
 * Applies a blanket policy to all city-park trails:
 *   dogsAllowed: "allowed", leashPolicy: "required", policyConfidence: 0.85
 *
 * City of Norman Municipal Code §18-78 requires dogs on leash in all city parks.
 * Off-leash only in designated dog park areas.
 *
 * DRY RUN by default — no DB writes until --commit is passed.
 *
 * Usage:
 *   npx tsx scripts/policy/seed-policy-norman.ts \
...
const cityFilter = typeof args.city === "string" ? args.city : "Norman";
...
const NORMAN_POLICY = {
  ...
  leashDetails: "Dogs must be on a leash in all Norman city parks and trails. Off-leash only in designated dog park areas.",
  policySourceUrl: "https://www.normanok.gov/public-works/parks-recreation",
  policySourceTitle: "Parks & Recreation | City of Norman, OK",
  policyConfidence: 0.85,
  policyMethod: "manual_seed" as const,
  policyNotes: "City of Norman Municipal Code §18-78 requires dogs on leash in all city parks. Off-leash permitted only in designated off-leash areas.",
};

const RESTRICTED_SLUGS = new Set<string>([]);

async function main(): Promise<void> {
  console.log("=== seed-policy-norman ===");
  ...
  console.log(`Found ${systems.length} Norman trailSystems.\n`);
  ...
  for (const system of systems) {
    ...
    const alreadySet =
      system.dogsAllowed === NORMAN_POLICY.dogsAllowed &&
      system.leashPolicy === NORMAN_POLICY.leashPolicy &&
      system.policyConfidence === NORMAN_POLICY.policyConfidence &&
      system.policySourceUrl === NORMAN_POLICY.policySourceUrl;
  ...
  const payload: Record<string, unknown> = {
    dogsAllowed: NORMAN_POLICY.dogsAllowed,
    leashPolicy: NORMAN_POLICY.leashPolicy,
    leashDetails: NORMAN_POLICY.leashDetails,
    policySourceUrl: NORMAN_POLICY.policySourceUrl,
    policySourceTitle: NORMAN_POLICY.policySourceTitle,
    policyConfidence: NORMAN_POLICY.policyConfidence,
    policyMethod: NORMAN_POLICY.policyMethod,
    policyNotes: NORMAN_POLICY.policyNotes,
