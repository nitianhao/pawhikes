#!/usr/bin/env npx tsx
/**
 * Seed official dog policy fields onto Albany, NY trailSystems.
 *
 * DRY RUN by default — no DB writes until --commit is passed.
 *
 * Usage:
 *   npx tsx scripts/policy/seed-policy-albany.ts [--commit]
 */
...
const DEFAULT_POLICY: Policy = {
  ...
  leashDetails: "Dogs must be on a leash no longer than 6 feet in Albany city parks. Albany City Code §175. Dogs must be licensed and vaccinated. Owners must clean up after their dogs.",
  policySourceUrl: "https://www.albanyny.gov/government/departments/parks_and_recreation",
  policySourceTitle: "Parks & Recreation | City of Albany",
  policyConfidence: 0.78,
  policyMethod: "manual_seed",
  policyNotes: "Albany city parks require 6-ft leash. Corning Preserve (riverfront park) allows leashed dogs. Washington Park allows leashed dogs. Pine Bush Preserve allows leashed dogs on designated trails. Hudson-Mohawk Bike Trail allows leashed dogs.",
};

async function main(): Promise<void> {
  console.log("=== seed-policy-albany ===");
  ...
  console.log(`Found ${systems.length} Albany trailSystems.\n`);
