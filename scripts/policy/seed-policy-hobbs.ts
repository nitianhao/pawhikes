#!/usr/bin/env npx tsx
/**
 * Seed official dog policy fields onto Hobbs, NM trailSystems.
 * DRY RUN by default — no DB writes until --commit is passed.
 * Usage: npx tsx scripts/policy/seed-policy-hobbs.ts [--commit]
 */
...
const DEFAULT_POLICY: Policy = {
  ...
  leashDetails: "Dogs must be on leash in Hobbs city parks and Harry McAdams Park trail network. Hobbs City Code §6-2.",
  policySourceUrl: "https://www.hobbsnm.org/departments/parks_and_recreation",
  policySourceTitle: "Parks and Recreation | City of Hobbs",
  policyConfidence: 0.75,
  policyMethod: "manual_seed",
  policyNotes: "Hobbs City Code §6-2 requires leash at all times in city parks.",
};

async function main(): Promise<void> {
  console.log("=== seed-policy-hobbs ===");
