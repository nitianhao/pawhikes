#!/usr/bin/env npx tsx
/**
 * Seed official dog policy fields onto Rio Rancho, NM trailSystems.
 * DRY RUN by default — no DB writes until --commit is passed.
 * Usage: npx tsx scripts/policy/seed-policy-rio-rancho.ts [--commit]
 */
...
const DEFAULT_POLICY: Policy = {
  ...
  leashDetails: "Dogs must be on leash in Rio Rancho city parks and open space trails. Rio Rancho City Code §10-1-9.",
  policySourceUrl: "https://www.rrnm.gov/193/Parks-Recreation",
  policySourceTitle: "Parks and Recreation | City of Rio Rancho",
  policyConfidence: 0.75,
  policyMethod: "manual_seed",
  policyNotes: "Rio Rancho City Code §10-1-9 requires leash at all times in city parks and trails.",
};

async function main(): Promise<void> {
  console.log("=== seed-policy-rio-rancho ===");
