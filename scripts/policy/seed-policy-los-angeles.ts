#!/usr/bin/env npx tsx
/**
 * Seed official dog policy fields onto Los Angeles, CA trailSystems.
 * Applies LA_DEFAULT_POLICY to all systems in the city.
 *
 * DRY RUN by default — no DB writes until --commit is passed.
 *
 * Usage:
 *   npx tsx scripts/policy/seed-policy-los-angeles.ts [--commit]
 */
...
const DEFAULT_POLICY: Policy = {
  ...
  leashDetails: "Dogs must be on a leash no longer than 6 feet in all Los Angeles city and county parks. LAMC §53.06.2.",
  policySourceUrl: "https://www.laparks.org/rules",
  policySourceTitle: "Park Rules | City of Los Angeles Department of Recreation and Parks",
  policyConfidence: 0.80,
  policyMethod: "manual_seed",
  policyNotes: "LA city parks require 6-ft leash. Runyon Canyon has designated off-leash hours in upper park. Griffith Park leash required throughout.",
};

async function main(): Promise<void> {
  console.log("=== seed-policy-los-angeles ===");
  ...
  console.log(`Found ${systems.length} Los Angeles trailSystems.\n`);
