#!/usr/bin/env npx tsx
/**
 * Seed official dog policy fields onto Fresno, CA trailSystems.
 *
 * DRY RUN by default — no DB writes until --commit is passed.
 *
 * Usage:
 *   npx tsx scripts/policy/seed-policy-fresno.ts [--commit]
 */
...
const DEFAULT_POLICY: Policy = {
  ...
  leashDetails: "Dogs must be on a leash in all Fresno city parks. Fresno Municipal Code §7-2704.",
  policySourceUrl: "https://www.fresno.gov/parks/",
  policySourceTitle: "Parks, After School, Recreation and Community Services | City of Fresno",
  policyConfidence: 0.80,
  policyMethod: "manual_seed",
  policyNotes: "Fresno city parks require leash. Woodward Park and other regional parks allow leashed dogs on trails. Designated off-leash dog parks available at various locations.",
};

async function main(): Promise<void> {
  console.log("=== seed-policy-fresno ===");
  ...
  console.log(`Found ${systems.length} Fresno trailSystems.\n`);
