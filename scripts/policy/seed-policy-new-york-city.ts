#!/usr/bin/env npx tsx
/**
 * Seed official dog policy fields onto New York City, NY trailSystems.
 *
 * DRY RUN by default — no DB writes until --commit is passed.
 *
 * Usage:
 *   npx tsx scripts/policy/seed-policy-new-york-city.ts [--commit]
 */
...
const cityFilter = "New York City";
const stateFilter = "NY";
...
const DEFAULT_POLICY: Policy = {
  ...
  leashDetails: "Dogs must be on a leash no longer than 6 feet in all NYC parks. NYC Admin Code §17-342. Off-leash hours allowed in designated areas of many parks from 9 PM–9 AM and before 9 AM.",
  policySourceUrl: "https://www.nycgovparks.org/facilities/dogrun",
  policySourceTitle: "Dog Runs & Off-Leash Areas | NYC Parks",
  policyConfidence: 0.90,
  policyMethod: "manual_seed",
  policyNotes: "NYC parks generally require 6-ft leash. Many parks offer designated off-leash areas and dog runs. Central Park has off-leash hours in designated meadows. Prospect Park has designated off-leash areas. Dogs not permitted on beaches Memorial Day–Labor Day.",
};

async function main(): Promise<void> {
  console.log("=== seed-policy-new-york-city ===");
  ...
  console.log(`Found ${systems.length} New York City trailSystems.\n`);
