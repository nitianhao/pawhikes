#!/usr/bin/env npx tsx
/**
 * Seed official dog policy fields onto Utica, NY trailSystems.
 *
 * DRY RUN by default — no DB writes until --commit is passed.
 *
 * Usage:
 *   npx tsx scripts/policy/seed-policy-utica.ts [--commit]
 */
...
const DEFAULT_POLICY: Policy = {
  ...
  leashDetails: "Dogs must be on a leash no longer than 6 feet in Utica city parks. Dogs must be licensed and vaccinated. Owners are responsible for cleanup.",
  policySourceUrl: "https://www.uticany.gov/departments/parks-recreation",
  policySourceTitle: "Parks & Recreation | City of Utica",
  policyConfidence: 0.73,
  policyMethod: "manual_seed",
  policyNotes: "Utica city parks require leashed dogs. Roscoe Conkling Park allows leashed dogs on trails. Proctor Park allows leashed dogs. Mohawk River trail corridor accessible from the city. Erie Canalway Trail (Oneida County) allows leashed dogs.",
};

async function main(): Promise<void> {
  console.log("=== seed-policy-utica ===");
  ...
  console.log(`Found ${systems.length} Utica trailSystems.\n`);
