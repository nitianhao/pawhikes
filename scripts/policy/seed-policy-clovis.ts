#!/usr/bin/env npx tsx
/**
 * Seed official dog policy fields onto Clovis, NM trailSystems.
 * DRY RUN by default — no DB writes until --commit is passed.
 * Usage: npx tsx scripts/policy/seed-policy-clovis.ts [--commit]
 */
...
const DEFAULT_POLICY: Policy = {
  ...
  leashDetails: "Dogs must be on leash in Clovis city parks and recreation areas. Clovis City Code §6-1.",
  policySourceUrl: "https://www.cityofclovis.org/parks-recreation",
  policySourceTitle: "Parks and Recreation | City of Clovis",
  policyConfidence: 0.75,
  policyMethod: "manual_seed",
  policyNotes: "Clovis City Code §6-1 requires leash at all times in city parks.",
};

async function main(): Promise<void> {
  console.log("=== seed-policy-clovis ===");
