#!/usr/bin/env npx tsx
/**
 * Seed official dog policy fields onto Sierra Vista, AZ trailSystems.
 * Applies SIERRA_VISTA_DEFAULT_POLICY to all systems in the city.
 *
 * DRY RUN by default — no DB writes until --commit is passed.
 *
 * Usage:
 *   npx tsx scripts/policy/seed-policy-sierra-vista.ts [--commit]
 */
...
const cityFilter = "Sierra Vista";
const stateFilter = "AZ";
...
const DEFAULT_POLICY: Policy = {
  ...
  leashDetails: "Dogs must be on leash in all Sierra Vista city parks. Sierra Vista City Code §6.04.010. San Pedro Riparian National Conservation Area also requires leash.",
  policySourceUrl: "https://www.sierravistaaz.gov/government/departments/parks-recreation",
  policySourceTitle: "Parks and Recreation | City of Sierra Vista",
  policyConfidence: 0.80,
  policyMethod: "manual_seed",
  policyNotes: "Sierra Vista City Code §6.04.010 requires leash. San Pedro Riparian NCA (BLM) also requires leash.",
};

async function main(): Promise<void> {
  console.log("=== seed-policy-sierra-vista ===");
  ...
  console.log(`Found ${systems.length} Sierra Vista trailSystems.\n`);
