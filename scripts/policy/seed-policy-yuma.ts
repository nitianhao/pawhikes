#!/usr/bin/env npx tsx
/**
 * Seed official dog policy fields onto Yuma, AZ trailSystems.
 * Applies YUMA_DEFAULT_POLICY to all systems in the city.
 *
 * DRY RUN by default — no DB writes until --commit is passed.
 *
 * Usage:
 *   npx tsx scripts/policy/seed-policy-yuma.ts [--commit]
 */
...
const DEFAULT_POLICY: Policy = {
  ...
  leashDetails: "Dogs must be on leash in all Yuma city parks and along the Colorado River trail. Yuma City Code §6-3-6.",
  policySourceUrl: "https://www.yumaaz.gov/parks-recreation/pages/trails.aspx",
  policySourceTitle: "Trails | City of Yuma Parks and Recreation",
  policyConfidence: 0.80,
  policyMethod: "manual_seed",
  policyNotes: "Yuma City Code §6-3-6 requires leash in parks.",
};

async function main(): Promise<void> {
  console.log("=== seed-policy-yuma ===");
  ...
  console.log(`Found ${systems.length} Yuma trailSystems.\n`);
