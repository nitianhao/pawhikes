#!/usr/bin/env npx tsx
/**
 * Seed official dog policy fields onto New Rochelle, NY trailSystems.
 *
 * DRY RUN by default — no DB writes until --commit is passed.
 *
 * Usage:
 *   npx tsx scripts/policy/seed-policy-new-rochelle.ts [--commit]
 */
...
const DEFAULT_POLICY: Policy = {
  ...
  leashDetails: "Dogs must be on a leash no longer than 6 feet in New Rochelle city parks. Westchester County parks require 6-ft leash. Dogs must be licensed and vaccinated.",
  policySourceUrl: "https://www.newrochelleny.com/parks",
  policySourceTitle: "Parks | City of New Rochelle",
  policyConfidence: 0.75,
  policyMethod: "manual_seed",
  policyNotes: "New Rochelle parks require leashed dogs. Glen Island Park (Westchester County) allows leashed dogs. Twin Lakes Park allows leashed dogs on trails. Marshlands Conservancy allows leashed dogs. Hudson River Greenway accessible from city.",
};

async function main(): Promise<void> {
  console.log("=== seed-policy-new-rochelle ===");
  ...
  console.log(`Found ${systems.length} New Rochelle trailSystems.\n`);
