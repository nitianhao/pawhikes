#!/usr/bin/env npx tsx
/**
 * Seed official dog policy fields onto San Jose, CA trailSystems.
 *
 * DRY RUN by default — no DB writes until --commit is passed.
 *
 * Usage:
 *   npx tsx scripts/policy/seed-policy-san-jose.ts [--commit]
 */
...
const DEFAULT_POLICY: Policy = {
  ...
  leashDetails: "Dogs must be on a leash no longer than 6 feet in all San Jose city parks. SJMC §13.44.010.",
  policySourceUrl: "https://www.sanjoseca.gov/your-government/departments-offices/parks-recreation-neighborhood-services/dogs-in-parks",
  policySourceTitle: "Dogs in Parks | City of San Jose",
  policyConfidence: 0.80,
  policyMethod: "manual_seed",
  policyNotes: "San Jose city parks require 6-ft leash. Several off-leash dog parks exist (Hellyer, Fontana, Watson). Santa Clara County open space preserves generally allow leashed dogs on designated trails.",
};

async function main(): Promise<void> {
  console.log("=== seed-policy-san-jose ===");
  ...
  console.log(`Found ${systems.length} San Jose trailSystems.\n`);
