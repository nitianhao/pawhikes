#!/usr/bin/env npx tsx
/**
 * Seed official dog policy fields onto Flagstaff, AZ trailSystems.
 * Applies FLAGSTAFF_DEFAULT_POLICY to all systems in the city.
 *
 * DRY RUN by default — no DB writes until --commit is passed.
 *
 * Usage:
 *   npx tsx scripts/policy/seed-policy-flagstaff.ts [--commit]
 */
...
const DEFAULT_POLICY: Policy = {
  ...
  leashDetails: "Dogs must be on leash (max 6 feet) in all Flagstaff city parks and trails. Flagstaff City Code §5-08-001-0005.",
  policySourceUrl: "https://www.flagstaff.az.gov/1217/Parks-Recreation",
  policySourceTitle: "Parks and Recreation | City of Flagstaff",
  policyConfidence: 0.80,
  policyMethod: "manual_seed",
  policyNotes: "Flagstaff City Code requires 6-ft leash in parks.",
};

async function main(): Promise<void> {
  console.log("=== seed-policy-flagstaff ===");
  ...
  console.log(`Found ${systems.length} Flagstaff trailSystems.\n`);
