#!/usr/bin/env npx tsx
/**
 * Seed official dog policy fields onto Casa Grande, AZ trailSystems.
 * Applies CASA_GRANDE_DEFAULT_POLICY to all systems in the city.
 *
 * DRY RUN by default — no DB writes until --commit is passed.
 *
 * Usage:
 *   npx tsx scripts/policy/seed-policy-casa-grande.ts [--commit]
 */
...
const DEFAULT_POLICY: Policy = {
  ...
  leashDetails: "Dogs must be on leash in all Casa Grande city parks. Casa Grande City Code §6-1-2.",
  policySourceUrl: "https://www.casagrandeaz.gov/departments/parks-recreation",
  policySourceTitle: "Parks and Recreation | City of Casa Grande",
  policyConfidence: 0.75,
  policyMethod: "manual_seed",
  policyNotes: "Casa Grande City Code §6-1-2 requires leash in parks.",
};

async function main(): Promise<void> {
  console.log("=== seed-policy-casa-grande ===");
  ...
  console.log(`Found ${systems.length} Casa Grande trailSystems.\n`);
