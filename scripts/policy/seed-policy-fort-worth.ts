#!/usr/bin/env npx tsx
/**
 * Seed official dog policy fields onto Fort Worth trailSystems.
 * Applies a blanket policy to all city-park trails:
 *   dogsAllowed: "allowed", leashPolicy: "required", policyConfidence: 0.85
 *
 * Fort Worth City Code Chapter 6 requires dogs on leash (max 6 ft) in all
 * city parks. Off-leash only in designated dog park areas.
 *
 * DRY RUN by default — no DB writes until --commit is passed.
 *
 * Usage:
 *   npx tsx scripts/policy/seed-policy-fort-worth.ts \
...
const cityFilter = typeof args.city === "string" ? args.city : "Fort Worth";
...
const FORT_WORTH_POLICY = {
  dogsAllowed: "allowed" as const,
  leashPolicy: "required" as const,
  leashDetails: "Dogs must be on a leash (max 6 feet) in all Fort Worth city parks. Owner must hold the leash at all times.",
  policySourceUrl: "https://www.fortworthtexas.gov/departments/code-compliance/animals/pet-resources/animal-control-laws",
  policySourceTitle: "Animal Control Laws | Fort Worth, TX",
  policyConfidence: 0.85,
  policyMethod: "manual_seed" as const,
  policyNotes: "Fort Worth City Code Chapter 6 requires dogs on leash (max 6 ft) in all city parks. Off-leash only in designated dog park zones.",
};

async function main(): Promise<void> {
  console.log("=== seed-policy-fort-worth ===");
  ...
  console.log(`Found ${systems.length} Fort Worth trailSystems.\n`);
  ...
  for (const system of systems) {
    ...
    const alreadySet =
      system.dogsAllowed === FORT_WORTH_POLICY.dogsAllowed &&
      system.leashPolicy === FORT_WORTH_POLICY.leashPolicy &&
      system.policyConfidence === FORT_WORTH_POLICY.policyConfidence;
  ...
  const payload: Record<string, unknown> = {
    dogsAllowed: FORT_WORTH_POLICY.dogsAllowed,
    leashPolicy: FORT_WORTH_POLICY.leashPolicy,
    leashDetails: FORT_WORTH_POLICY.leashDetails,
    policySourceUrl: FORT_WORTH_POLICY.policySourceUrl,
    policySourceTitle: FORT_WORTH_POLICY.policySourceTitle,
    policyConfidence: FORT_WORTH_POLICY.policyConfidence,
    policyMethod: FORT_WORTH_POLICY.policyMethod,
    policyNotes: FORT_WORTH_POLICY.policyNotes,
