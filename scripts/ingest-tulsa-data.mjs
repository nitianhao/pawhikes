#!/usr/bin/env node
/**
 * Ingest Tulsa, OK trail segments from OpenStreetMap via Overpass API.
...
// Tulsa, OK bbox: S,W,N,E — covers River Parks, Turkey Mountain, Osage Hills
const CITY_BBOX = "35.95,-96.10,36.30,-95.75";
const CITY_NAME = "Tulsa";
const CITY_STATE = "OK";
const CITY_COUNTY = "Tulsa";
const EXT_DATASET = "tulsa_osm";
...
const TRAIL_INDICATORS = /\b(trail|trails|greenway|greenways|path|hike|bikeway|corridor|lake|creek|river|mountain|park)\b/i;
...
const NORMALIZE_NAMES = new Map([
  // Turkey Mountain
  ["turkey mountain trail", "Turkey Mountain Trail"],
  // River Parks
  ["river parks trail", "River Parks Trail"],
  ["river parks east bank trail", "River Parks Trail"],
  ["river parks west bank trail", "River Parks Trail"],
]);
...
async function fetchOSMTrails() {
  const trailNameRegex = "trail|trails|greenway|greenways|hike|bikeway|corridor|lake|creek|river|mountain|park";
  const query = [
